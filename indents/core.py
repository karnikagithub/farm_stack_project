...
async def create_indent_and_items(indent_data: IndentDetails):
    ...
    #     raise HTTPException(status_code=400, detail="Department doesn't exists")
    ...
    # # Check the existence of each item
    ...
    # # Check if the indent details were inserted successfully
    # if not created_indent:
    #     raise HTTPException(status_code=500, detail="Failed to create indent details")
    

    # # Ensure that created_indent is a dictionary and has the necessary fields
    # if not isinstance(created_indent, dict) or 'indent_purpose' not in created_indent:
    #     raise HTTPException(status_code=500, detail="Invalid data format retrieved")
    
    
    # # Check if the indent_code already exists in the collection
    ...
    # created_indent_model = IndentDetails(
    #     indent_purpose=created_indent['indent_purpose'],
    #     indent_remarks=created_indent['indent_remarks'],
    #     indent_code=created_indent['indent_code'],
    #     department_id=created_indent['department_id'],
    #     location_id=created_indent['location_id'],
    #     indentitems=[ItemDetails(**item) for item in created_indent.get('indentitems', [])],
    #     status=created_indent['status']
    # )
...
# @indent_router.get("/indent_details", response_model=IndentDetailsView)
# async def indent_details_view():

#     indent_collect = mongo_client("your_db_name", "indent_details")

#     # Retrieve all indents
#     all_indents = list(indent_collect.find({}))

#     # Convert each MongoDB document to IndentDetails model
#     indent_details_list = [IndentDetails(**indent) for indent in all_indents]

#     # Create and return the IndentDetailsView
#     return IndentDetailsView(indent_details=indent_details_list)
